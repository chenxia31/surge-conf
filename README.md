...
| | 方式一 自建主配置 | 方式二 托管配置 + 模块 |
|---|---|---|
| 主配置归属 | 你 | 机场 |
| 分流规则 | 全部自己写 | 机场的 + 模块追加 |
| 机场的流媒体解锁 / 策略组 | 需自己维护 | 直接沿用 |
| 机场的 MITM 脚本、全局 QUIC 拦截 | 不会引入 | 一并继承 |
| 凭证（订阅 token） | 需自行安置，不能进公开仓库 | 不接触 |
| 迁移成本 | 中 | 零 |


### 方式一：自建主配置 + policy-path 引入机场节点

机场只提供节点，主配置、分流规则、模块全部由本仓库掌握。控制力最强，也能避开机场配置里不需要的部分（全局 QUIC 拦截、第三方 MITM 脚本等）。

`surge.conf` 的 `[Proxy Group]` 已按此形态写好，使用前把 `<SUB_URL>` 替换为机场订阅链接：

```
HK = select, policy-path=<SUB_URL>&list=true, policy-regex-filter=香港, update-interval=86400
```

- **`&list=true` 必须加**：subconverter 默认返回整份 conf，`policy-path` 只能解析纯节点列表。加上后在浏览器打开应看到一行行节点，没有 `[General]`。
- `policy-regex-filter` 按地区名匹配，机场展示流量/到期用的伪节点（`Traffic: …` / `Expire: …`）会被自动排除。
- 节点名保持机场下发原样，只做分组（见 HANDOFF 第 9 节）。

**凭证处理**：订阅链接含 token，本仓库是 public，不能提交真实链接。两种做法：

1. 主配置只存在设备本地，`[Rule]` / `[Module]` 通过 Raw 链接引用本仓库 —— 规则仍然版本化，凭证不外泄（推荐）。
2. 主配置放 secret gist，Surge 订阅 gist 的 raw 链接。

### 方式二：机场托管配置 + 本仓库模块覆写

零迁移成本。保留机场托管配置（节点、流媒体解锁、策略组全部照旧），只用模块追加规则。

机场下发的托管配置（首行含 `#!MANAGED-CONFIG`）由订阅管理，`strict=true` 时 Surge 直接禁止编辑，`strict=false` 时手改也会在下次更新被覆盖。此时不要改机场配置，改为安装本仓库的覆写模块。
